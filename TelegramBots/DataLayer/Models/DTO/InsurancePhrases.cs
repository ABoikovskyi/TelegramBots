namespace DataLayer.Models.DTO
{
	public class InsurancePhrases
	{
		public string StartText { get; set; }
		public string StartFromBegining { get; set; }
		public string Operation1 { get; set; }
		public string Operation2 { get; set; }
		public string Operation3 { get; set; }
		public string Operation4 { get; set; }
		public string Operation5 { get; set; }
		public string Contacts { get; set; }
		public string Operation1Step1 { get; set; }
		public string Operation1Step2 { get; set; }
		public string Operation1Step3 { get; set; }
		public string Operation1Step4 { get; set; }
		public string Operation1Step5 { get; set; }
		public string Operation1End { get; set; }
		public string Operation2Step1 { get; set; }
		public string Operation2Step2 { get; set; }
		public string Operation2Step3 { get; set; }
		public string Operation2Step4 { get; set; }
		public string Operation2End { get; set; }
		public string Operation3Step1 { get; set; }
		public string Operation3End { get; set; }
		public string Operation4Step1 { get; set; }
		public string Operation4Step2 { get; set; }
		public string Operation4End { get; set; }
		public string Operation3DocumentsText { get; set; }
		public string Operation3AddressForDocuments { get; set; }
		public string SendDocumentsToMail { get; set; }
	}
}